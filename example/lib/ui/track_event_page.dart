...
class _State extends State<TrackEventPage> {
  ...
  String properties = "{\"textProp\":\"Hello world!\",\"integerProp\":123456,\"fpProp\":123.456,\"boolProp\":true}";
  int repeat = 1;
  int interval = 0;
                ...
                children: [
                  Flexible(
                    child: TextField(
                      ...
                      onChanged: (str) => setState(() => repeat = int.tryParse(str) ?? repeat),
                      maxLines: null,
                  ...
                  Flexible(
                      child: TextField(
                        ...
                        onChanged: (str) => setState(() => interval = int.tryParse(str) ?? interval),
                        maxLines: null,
                        keyboardType: TextInputType.number,
                        inputFormatters: <TextInputFormatter>[
                          FilteringTextInputFormatter.digitsOnly
...
class _RunButtonState extends State<_RunButton> {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        Text("Times of repeat: $repeated / ${widget.repeats > 1? widget.repeats : 1}"),
          ...
          onPressed: () {
            ...
            if (widget.eventName.isNotEmpty) {
              final repeats = widget.repeats > 1 ? widget.repeats : 1;
                ...
                Future.delayed(Duration(milliseconds: widget.interval * (i-1)), () {
                    DTAnalytics.trackEvent(
                        widget.eventName,
                        widget.properties.isEmpty ? null : jsonDecode(widget.properties)
                    );
                    setState(() { repeated = i; });
                });
              }
